...
public sealed class CosmosBootstrapper(CosmosClient client, IConfiguration config, ILogger<CosmosBootstrapper> logger) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("Cosmos client endpoint: {Endpoint}", client.Endpoint);
            ...
            var gamesVectorPath = config["Cosmos:GamesVector:Path"] ?? "/gvector";
            ...
            // Ensure games container exists with vector policy and index (for game-level embeddings)
            var gameEmbeddings = new List<Embedding>
            {
                new Embedding
                {
            ...
            {
                VectorEmbeddingPolicy = new VectorEmbeddingPolicy(new System.Collections.ObjectModel.Collection<Embedding>(gameEmbeddings)),
        ...
        {
            logger.LogError(ex, "Cosmos bootstrap failed (non-fatal): {Message}", ex.Message);
        }
    }
